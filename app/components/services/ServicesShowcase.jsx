      ...
      <div className='max-w-6xl mx-auto'>
        ...
        <div className='flex gap-6 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide'>
          {services.map((service, index) => (
            <div
              key={service.title}
              className={`shrink-0 w-72 md:w-80 snap-start bg-white rounded-2xl  transition-all duration-700 ease-out ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
              style={{ transitionDelay: visible ? `${index * 120}ms` : '0ms' }}
            >
              {/* Image with arrow link */}
              <div className='relative h-56 rounded-2xl overflow-hidden m-3'>
                ...
                <Link
                  ...
                  className='group absolute bottom-3 right-3 w-10 h-10 rounded-full bg-linear-to-r from-amber-400 to-orange-600 flex items-center justify-center text-white shadow-lg overflow-hidden hover:scale-110 transition-transform'
                >
                  {/* Default arrow — slides out on hover */}
                  <ArrowRight
                    size={18}
                    className='absolute transition-all duration-300 group-hover:translate-x-6 group-hover:opacity-0'
                  />
                  {/* Up-right arrow — slides in on hover */}
                  <ArrowUpRight
                    size={18}
                    className='absolute -translate-x-6 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100'
                  />
                </Link>
              </div>
